...
static bool ipcScanCmd(int argc, char *argv[], bool fRelay)
{
    ...
    {
        if (boost::algorithm::istarts_with(argv[i], "fucoin:"))
        {
...
void ipcInit(int argc, char *argv[])
{
    ...
    try {
        ...
        // Make sure we don't lose any fucoin: URIs
        for (int i = 0; i < 2; i++)
        {
        ...
        // Make sure only one fucoin instance is listening
        message_queue::remove(BITCOINURI_QUEUE_NAME);
        delete mq;
